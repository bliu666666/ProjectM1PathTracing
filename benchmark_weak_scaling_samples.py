...
def run_benchmark(width, height, samples, depth, threads, output):
    ...
    print(f"  Lancement avec {threads} threads et {samples} échantillons: {' '.join(cmd)}")
...
def main():
    parser = argparse.ArgumentParser(description="Mesure du weak scaling - augmente le nombre d'échantillons proportionnellement au nombre de threads")
    
    # Arguments pour la configuration du benchmark
    parser.add_argument('--width', type=int, default=200,
                       help="Largeur fixe de l'image")
    parser.add_argument('--height', type=int, default=200,
                       help="Hauteur fixe de l'image")
    parser.add_argument('--base-samples', type=int, default=100,
                       help="Nombre d'échantillons de base pour 1 thread")
    ...
    parser.add_argument('--output-dir', type=str, default='benchmarks_weak_scaling_samples',
    ...
    print(f"Configuration: image fixe {args.width}x{args.height}, {args.base_samples} échantillons de base")
    
    for threads in thread_counts:
        # Calculer le nombre d'échantillons proportionnel au nombre de threads
        samples = args.base_samples * threads
        print(f"\nTest avec {threads} threads - Échantillons: {samples}")
    ...
    results_file = os.path.join(args.output_dir, 'weak_scaling_samples_results.txt')
    with open(results_file, 'w') as f:
        f.write("Threads\tSamples\tTime(s)\n")
        for t, s, time in zip(valid_threads, samples_list, times):
    ...
    # Titre
    plt.title('Weak Scaling Performance\n(nombre d\'échantillons augmente avec le nombre de threads)')
    ...
    plt.tight_layout()
    plot_filename = os.path.join(args.output_dir, 'weak_scaling_samples_plot.png')
